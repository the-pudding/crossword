...
import raceGenderBreakdownByDecade from "../../data/raceGenderBreakdownByDecade.json";

const CensusLineChart = () => {
  ...
  const xScale = d3
    .scalePoint()
    .domain(usCensusData.map((d) => d.decade))
  ...
  return (
    <>
      <LineChartWrapper ref={ref}>
        <Chart dms={dms}>
          {[usCensusData, raceGenderBreakdownByDecade].map((data, i) => (
            <Line
              key={`line-${i}`}
              data={data}
      ...
      >
        ...
        <option value="whiteNonHispanic">whiteNonHispanic</option>
...
export default CensusLineChart;
